...
def generate_session_pdf(session_id):
    """Generate a visually appealing PDF for session details"""
    sessions = load_json(SESSIONS_FILE, {})
    ...
    # Title
    title = Paragraph("SESSION DETAILS REPORT", title_style)
    ...
    line_table.setStyle(TableStyle([
        ...
        ('LINEBELOW', (0, 0), (-1, -1), 3, colors.HexColor('#1f4788'))
    ]))
    elements.append(line_table)
    elements.append(Spacer(1, 0.3*inch))
    
    # Basic Information Section
    elements.append(Paragraph("Basic Information", header_style))
    
    # Create a table for basic info with nice formatting
    basic_info_data = [
        ['Session ID:', session['id']],
        ['Session Name:', session['session_name']],
        ['Faculty Name:', session['faculty_name']],
        ['Date:', session['date']],
        ['Duration:', f"{session['duration']} hours"],
        ['Batch:', session.get('batch', 'N/A')],
        ['Honorarium Paid:', f"₹{session.get('honorarium_paid', 0):,.2f}"]
    ]
    
    basic_info_table = Table(basic_info_data, colWidths=[2*inch, 5*inch])
    ...
    elements.append(Paragraph("Feedback Scores", header_style))
    ...
    # Summary box
    summary_data = [[
        Paragraph(f"<b>Total Score: {total_obtained:.2f} / {total_max:.0f}</b>", normal_style),
        Paragraph(f"<b>Percentage: {percentage:.1f}%</b>", normal_style)
    ]]
    ...
    feedback_data = [['#', 'Criteria', 'Score', 'Max', 'Percentage']]
    
    for i, (key, (label, max_marks)) in enumerate(feedback_labels.items(), 1):
        score = feedback.get(key, 0)
        score_percentage = (score / max_marks * 100) if max_marks > 0 else 0
    ...
    feedback_table = Table(feedback_data, colWidths=[0.5*inch, 4*inch, 0.8*inch, 0.8*inch, 1*inch])
    feedback_table.setStyle(TableStyle([
        ...
        ('ALIGN', (2, 1), (-1, -1), 'CENTER'),  # Score columns
        ('ALIGN', (1, 1), (1, -1), 'LEFT'),     # Criteria column
        ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
        ...
        # Highlight overall performance row
    ...
    if feedback.get('recommend_again'):
        ...
        recommendation_text = Paragraph(feedback['recommend_again'], normal_style)
        recommendation_data = [[recommendation_text]]
        
        recommendation_table = Table(recommendation_data, colWidths=[7*inch])
    ...
    elements.append(Paragraph("Faculty Management System", footer_style))
...
def manage_sessions_page():
    ...
    tab1, tab2, tab3 = st.tabs(["📝 Add Session", "📋 View/Edit Sessions", "🔍 Search Sessions"])
    ...
    with tab3:
        ...
        if not sessions:
            ...
            search_query = st.text_input(
                "Search by Session Name, Faculty Name, or Batch",
                placeholder="e.g., Machine Learning, Dr. Smith, Batch 2024",
            ...
            col1, col2, col3 = st.columns(3)
            with col1:
                search_in_session_name = st.checkbox("Search in Session Name", value=True)
            with col2:
                search_in_faculty = st.checkbox("Search in Faculty Name", value=True)
            with col3:
                search_in_batch = st.checkbox("Search in Batch", value=True)
            ...
            if search_query:
                ...
                for sid, session in sessions.items():
                    ...
                    if search_in_session_name and search_lower in session.get('session_name', '').lower():
                        match = True
                    if search_in_faculty and search_lower in session.get('faculty_name', '').lower():
                        match = True
                    if search_in_batch and search_lower in session.get('batch', '').lower():
                        match = True
                    
                    if match:
                        filtered_sessions[sid] = session
                
                # Display results
                if filtered_sessions:
                    st.success(f"Found {len(filtered_sessions)} session(s) matching '{search_query}'")
                    ...
                    sorted_filtered = sorted(filtered_sessions.items(), key=lambda x: x[1].get('date', ''), reverse=True)
                    
                    for sid, session in sorted_filtered:
                        with st.expander(f"📚 {session['session_name']} - {session['date']} ({session['id']})"):
                            col1, col2 = st.columns([2, 1])
                            ...
                            with col2:
                                if st.button("View Full Details", key=f"search_view_{sid}"):
                                    st.session_state.selected_session = sid
                                    st.session_state.page = 'session_details'
                                    st.rerun()
                else:
                    st.warning(f"No sessions found matching '{search_query}'")
                    st.info("Try adjusting your search query or enabling more search filters.")
            else:
                st.info("👆 Enter a search query above to find sessions.")
                ...
                with col1:
                    st.metric("Total Sessions", len(sessions))
                with col2:
                    unique_faculties = len(set(s.get('faculty_name', '') for s in sessions.values()))
                    st.metric("Unique Faculties", unique_faculties)
                with col3:
                    unique_batches = len(set(s.get('batch', '') for s in sessions.values() if s.get('batch')))
...
def session_details_page():
    ...
    with col1:
        if st.button("✏️ Edit Session", type="primary", width="stretch"):
    ...
    with col2:
        # Generate PDF button
        pdf_data = generate_session_pdf(session_id)
        if pdf_data:
            st.download_button(
                label="📄 Download as PDF",
                data=pdf_data,
                file_name=f"session_{session['id']}_{session['date']}.pdf",
...
def about_developer_page():
    ...
    with col2:
        # Header section with gradient-like effect using markdown
        st.markdown("""
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                    ...
                    margin-bottom: 30px;">
            <h1 style="color: white; margin: 0; font-size: 2.5em;">Abhinav Jha</h1>
            <p style="color: #f0f0f0; margin-top: 10px; font-size: 1.2em;">Software Developer & System Architect</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Contact card
        st.markdown("""
        <div style="background-color: rgba(102, 126, 234, 0.15); 
                    padding: 25px; 
                    border-radius: 10px; 
                    border-left: 5px solid #667eea;
                    margin-bottom: 20px;">
            <p style="font-size: 1.1em; margin: 10px 0; color: var(--text-color);">
                <strong>📧 Email:</strong> 
                <a href="mailto:neoeon05@gmail.com" style="color: #667eea; text-decoration: none;">
                    neoeon05@gmail.com
                </a>
            </p>
        </div>
        ...
        st.markdown("""
        <div style="background-color: rgba(232, 244, 248, 0.3); 
                    padding: 25px; 
                    border-radius: 10px;
                    border: 1px solid rgba(102, 126, 234, 0.3);
                    margin-bottom: 20px;">
            <p style="font-size: 1em; line-height: 1.8; color: var(--text-color);">
                The <strong>Faculty Management System</strong> is a comprehensive platform designed to streamline 
                the management of academic sessions, faculty information, and performance analytics. 
                Built with modern web technologies, this system offers:
            </p>
            <ul style="font-size: 1em; line-height: 1.8; color: var(--text-color);">
                <li>📊 Real-time session tracking and management</li>
                <li>👥 Faculty profile and performance monitoring</li>
                <li>📈 Advanced analytics and reporting capabilities</li>
                <li>📄 PDF export functionality for session details</li>
                <li>🔍 Powerful search and filter options</li>
                <li>📥 Bulk import for easy data migration</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col_tech1:
            st.markdown("""
            <div style="background-color: rgba(255, 243, 205, 0.2); 
                        padding: 20px; 
                        border-radius: 10px;
                        border: 1px solid rgba(255, 193, 7, 0.3);
                        margin-bottom: 10px;">
                <h4 style="color: #ffc107; margin-top: 0;">Frontend</h4>
                <p style="color: var(--text-color); margin: 5px 0;">• Streamlit</p>
                <p style="color: var(--text-color); margin: 5px 0;">• Python</p>
                <p style="color: var(--text-color); margin: 5px 0;">• HTML/CSS</p>
            </div>
            """, unsafe_allow_html=True)
        
        with col_tech2:
            st.markdown("""
            <div style="background-color: rgba(212, 237, 218, 0.2); 
                        padding: 20px; 
                        border-radius: 10px;
                        border: 1px solid rgba(40, 167, 69, 0.3);
                        margin-bottom: 10px;">
                <h4 style="color: #28a745; margin-top: 0;">Libraries</h4>
                <p style="color: var(--text-color); margin: 5px 0;">• Pandas</p>
                <p style="color: var(--text-color); margin: 5px 0;">• ReportLab</p>
                <p style="color: var(--text-color); margin: 5px 0;">• OpenpyXL</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        features = [
            ("🎯", "User-Friendly Interface", "Intuitive design for easy navigation and quick access"),
            ("🔒", "Secure Authentication", "Role-based access control for admins and users"),
            ("📊", "Comprehensive Analytics", "Detailed reports and visualizations for insights"),
            ("💾", "Data Management", "Efficient storage and retrieval of faculty and session data"),
        ...
        for icon, title, description in features:
            ...
            <div style="background-color: rgba(102, 126, 234, 0.1); 
                ...
                <p style="margin: 0;">
                    ...
                    <strong style="font-size: 1.1em; margin-left: 10px; color: var(--text-color);">{title}</strong>
                </p>
                <p style="color: var(--text-color); opacity: 0.8; margin: 5px 0 0 40px; font-size: 0.95em;">{description}</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        <div style="text-align: center; 
            ...
            <p style="margin-top: 10px;">© 2025 Abhinav Jha. All rights reserved.</p>
        </div>
        """, unsafe_allow_html=True)
...
def main():
    if not st.session_state.logged_in:
        ...
        with st.sidebar:
            ...
            if st.button("ℹ️ About Developer"):
                st.session_state.page = 'about_developer'
                st.rerun()
        ...
        elif st.session_state.page == 'about_developer':
